...
def build_note_dictionaries(note_names, add_octave_no=True):
    ...
    for octave in range(9):
        ...
        for note in range(NOTES_PER_OCTAVE):
            ...
            midi_number = C0_MIDI_NO + octave * NOTES_PER_OCTAVE + note
            # Ajouter le numéro de l'octave au nom de la note si add_octave_no est vrai
            note_name = note_names[note] + (str(octave) if add_octave_no else "")
            midi_to_name[midi_number] = note_name
            # Garder les numéros de notes dans name_to_midi entre 0 et 11 si add_octave_no est faux
            name_to_midi[note_name] = (
                midi_number if add_octave_no else midi_number % NOTES_PER_OCTAVE
            )
    return midi_to_name, name_to_midi


def build_print_note_name_callback(midi_to_name):
    def note_callback(midi_message):
        if midi_message.type == "note_on" and midi_message.velocity > 0:
            print(midi_to_name[midi_message.note])

    return note_callback


def build_print_chord_name_callback(chord_names_and_notes, name_to_midi):
    chords_status = {}
    for name in chord_names_and_notes:
        chords_status[name] = {}
        for note in chord_names_and_notes[name]:
            chords_status[name][name_to_midi[note]] = False

    def chord_callback(message):
        if message.type != "note_off" and message.type != "note_on":
            return

        is_on = message.type == "note_on" and message.velocity > 0

        for chord, notes in chords_status.items():
            for note in notes:
                if note == message.note:
                    if not is_on and False not in notes.values():
                        print("Stopping chord", chord)

                    chords_status[chord][note] = is_on

                    if is_on and False not in notes.values():
                        print("Starting chord", chord)

    return chord_callback


def main():
    PORT_MIDI = "Midi Through:Midi Through Port-0 14:0"
    ...
    midi_to_name_eng_8va, name_to_midi_eng_8va = build_note_dictionaries(
        english_names, True
    )
    ...
    chord_names = {
        "Do majeur": ("Do3", "Mi3", "Sol3"),
        "Fa majeur": ("Fa3", "La3", "Do3"),
        "Sol majeur": ("Sol3", "Si2", "Ré3"),
    }
